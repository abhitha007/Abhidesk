...
function resizeCanvas() {
  ...
  if (currentSessionId) renderStrokesFromCache();
}
...
// Drawing events
canvas.addEventListener('mousedown', e => { drawing = true; lastPosition = getPos(e); });
canvas.addEventListener('touchstart', e => { drawing = true; lastPosition = getPos(e); });
...
async function joinSession(sessionId) {
  ...
  if (!doc.exists) await sessionRef.set({ created: firebase.firestore.FieldValue.serverTimestamp() });
...
eraserButton.addEventListener('click', () => { ctx.strokeStyle = '#FFFFFF'; ctx.lineWidth = 20; });

clearButton.addEventListener('click', async () => {
  if (!currentSessionId) return alert("Please join a room first.");
